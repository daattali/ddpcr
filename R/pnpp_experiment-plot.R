  ...
  superimpose = FALSE, show_drops = TRUE, drops_size = 1)
{
  ...
  neg_drops_low_freq <-
    plate %>%
    plate_data %>%
    dplyr::filter(well %in% wells_positive(plate)) %>%
    dplyr::filter(cluster == cluster(plate, 'NEGATIVE'))
  ...
  if (show_low_high_neg_freq) {
    if (sum(meta_used[['success']], na.rm = TRUE) > 0) {
      p <- p +
        ggplot2::geom_rect(
          data = meta_used %>% dplyr::filter(success),
  ...
  if (show_negative_freq) {
    if (sum(meta_used[['success']], na.rm = TRUE) > 0) {
      ...
      p <- p +
        ggplot2::geom_text(
          data = dplyr::filter(meta_used, ~ success),
