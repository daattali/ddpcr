...
This document provides an explanation of what the package does includes a tutorial on how to use. It should take about 20 minutes to go through this document. 
...
So far we only used the default plate type in thuse analysis, but droplet gating does not take place with the default type. While you can define your own plate types (more on that later), there are three built-in plate types you can use:
...
For more details about the algorithm, see [the "Algorithms" vignette](algorithm.Rmd).
...
For more details about how to create new plate types, see [the "Extending ddpcr by adding new plate types" vignette](extend.Rmd).